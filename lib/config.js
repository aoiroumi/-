// このファイルは「案件進捗管理表_テンプレート.xlsx」をそのままGoogleスプレッドシート化した
// 前提でセル位置を定義しています。シートの行・列構成を変更した場合はここを合わせて修正してください。

export const ITEMS = [
  "所属へ提案した数",
  "カジュアル面談",
  "BP確保", // シート上では非表示行として運用（未使用）。行の並び自体は維持
  "エントリーした数",
  "顧客面談",
  "オファー",
  "受注額（万円）",
];

export const N_WEEKS = 5;

// 月ブロックごとの「月見出し行」（例：「9月」と書かれている行番号）。
// 2026年9月分から暦月表記（9月, 10月, 11月...）に運用を変更。
// 新しい月をシートに追加したら、ここにも { label, headerRow } を1行追記してください
// （例：11月なら { label: "11月", headerRow: 32 }）。
// headerRow の間隔は既存の月ブロックと同じ構成であれば 12 行ごとに増えていきます
// （見出し1行＋サブヘッダー2行＋項目7行＋空白2行＝12行）。
// この構成は、下の VIEWS に登録するどのシートタブでも共通です。
export const MONTHS = [
  { label: "9月", headerRow: 8 },
  { label: "10月", headerRow: 20 },
];

// 表示の切り替え候補。id は画面のドロップダウンや ?view= パラメータで使う識別子。
// sheetTab は、そのビューが読み書きするGoogleスプレッドシートの「タブ（シート）名」。
// メンバーを追加・変更したいときは、この配列に1行追加し、
// Googleスプレッドシート側にも同じ名前のタブを複製してください（行・列構成は既存タブと同じにする）。
export const VIEWS = [
  { id: "team", label: "チーム全体", sheetTab: "案件進捗管理表" },
  { id: "osone", label: "小曽根", sheetTab: "案件進捗管理表_小曽根" },
  { id: "ishikawa", label: "石川", sheetTab: "案件進捗管理表_石川" },
  { id: "ohno", label: "大野", sheetTab: "案件進捗管理表_大野" },
];

export const DEFAULT_VIEW_ID = VIEWS[0].id;

export function resolveView(viewId) {
  return VIEWS.find((v) => v.id === viewId) || VIEWS[0];
}

const FIRST_DATA_COL = 2; // B列

function colLetter(idx) {
  let s = "";
  while (idx > 0) {
    const rem = (idx - 1) % 26;
    s = String.fromCharCode(65 + rem) + s;
    idx = Math.floor((idx - 1) / 26);
  }
  return s;
}

// 各月ブロックの実座標（データ行・列）を計算して返す（どのシートタブでも共通の構成）
export function resolveMonthLayout(month) {
  const headerRow1 = month.headerRow + 1; // 月計/週 見出し行
  const headerRow2 = month.headerRow + 2; // 目標/進捗/達成率 見出し行
  const dataFirstRow = month.headerRow + 3;
  const dataLastRow = dataFirstRow + ITEMS.length - 1;

  const blockStarts = []; // [月計開始列, 週1開始列, 週2開始列, ...]
  let col = FIRST_DATA_COL;
  for (let i = 0; i < 1 + N_WEEKS; i++) {
    blockStarts.push(col);
    col += 3;
  }

  const monthTotal = { goalCol: blockStarts[0], progCol: blockStarts[0] + 1, rateCol: blockStarts[0] + 2 };
  const weeks = blockStarts.slice(1).map((c, i) => ({
    week: i + 1,
    goalCol: c,
    progCol: c + 1,
    rateCol: c + 2,
  }));

  const lastCol = blockStarts[blockStarts.length - 1] + 2;

  return {
    label: month.label,
    headerRow1,
    headerRow2,
    dataFirstRow,
    dataLastRow,
    monthTotal,
    weeks,
    lastCol,
  };
}

export function colLetterOf(idx) {
  return colLetter(idx);
}

// 指定したシートタブで読み書きする最大範囲（A1:{最終列}{最終行}）を返す
export function fullRangeA1(sheetTab) {
  const lastMonth = MONTHS[MONTHS.length - 1];
  const layout = resolveMonthLayout(lastMonth);
  return `${sheetTab}!A1:${colLetter(layout.lastCol)}${layout.dataLastRow}`;
}
